      ...
      <div className={`${outputClass} p-6 overflow-auto`}>
        ...
        `}>
          <div className="flex flex-col items-center space-y-4">
            <Trophy 
              size={64} 
              className={`${isDarkMode ? 'text-green-400' : 'text-green-600'} animate-bounce`} 
            />
            <h2 className={`
              text-2xl font-bold 
            ...
            `}>
              ...
              <div className="flex items-center space-x-2">
                <Trophy size={20} />
                <span>Points: {selectedProblem.points || 100}</span>
              </div>
            </div>
            <p className={`
              text-lg font-medium 
      ...
      <div className="p-4 pt-2">
        {testCases.length > 0 && !isRunning ? (
          ...
          <div className={`${isDarkMode ? "bg-[#1e1e1e]" : "bg-white"} p-4 rounded-md text-center`}>
            {isRunning ? <div>{output}</div> : <div>Run your code to see test results</div>}
          </div>
        )}
      </div>
    </div>
